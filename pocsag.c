...
void prepare_msg_bin(struct l2_state_pocsag *rx, char* buff, unsigned char *rawbuf, int *rawsize)
{
    int len = rx->numnibbles;
...
static void pocsag_printmessage(struct demod_state *s, bool sync)
{
    ...
    if((s->l2.pocsag.address != -1) || (s->l2.pocsag.function != -1))
    {
        ...
        static char string_buffer[12288];
        ...
        if(s->l2.pocsag.numnibbles > 0)
        {
            switch (s->l2.pocsag.function)
            {
                case POCSAG_MODE_NUMERIC:
                    prepare_msg_numeric(&s->l2.pocsag, string_buffer, sizeof(string_buffer));
                    verbprintf(0, "Numeric: %s", string_buffer);
                    break;
                case POCSAG_MODE_BIN1:
                case POCSAG_MODE_BIN2:
                    prepare_msg_bin(&s->l2.pocsag, string_buffer, rawmsg, &rawsize);
                    verbprintf(0, "Hex:   %s  ", string_buffer);
                    break;
                case POCSAG_MODE_ALPHA:
                    prepare_msg_alpha(&s->l2.pocsag, string_buffer, sizeof(string_buffer), rawmsg, &rawsize);
                    verbprintf(0, "Alpha:   %s  ", string_buffer);
                    break;
            }
